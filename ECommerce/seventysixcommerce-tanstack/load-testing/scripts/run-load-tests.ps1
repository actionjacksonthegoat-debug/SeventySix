# run-load-tests.ps1
# Windows orchestrator for SeventySixCommerce TanStack load tests
#
# Usage:
...
Write-Host "  TanStack Load Tests ($TestProfile)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "Starting TanStack load test environment..." -ForegroundColor Cyan
Push-Location $RepoRoot
try {
	docker compose -f docker-compose.loadtest-tanstack.yml up -d --build
}
catch {
...
Write-Host ""
Write-Host "Waiting for TanStack app to become healthy..." -ForegroundColor Cyan
$maxAttempts = 60
$attempt = 0
$appHealthy = $false
...
while ($attempt -lt $maxAttempts) {
	$attempt++
	try {
		$response = Invoke-WebRequest -Uri "http://localhost:3022/api/healthz" -TimeoutSec 5 -ErrorAction SilentlyContinue -UseBasicParsing
...
if (-not $appHealthy) {
	...
	if (-not $KeepRunning) {
		Push-Location $RepoRoot
		docker compose -f docker-compose.loadtest-tanstack.yml down
		Pop-Location
	}
	exit 1
}

Write-Host "TanStack app is healthy!" -ForegroundColor Green
...
# 6. Print summary report path (never auto-open browser — causes VS Code/Copilot terminal freezes)
...
if (-not $KeepRunning) {
	...
	Write-Host "Stop manually: docker compose -f docker-compose.loadtest-tanstack.yml down" -ForegroundColor Yellow
}
