...
export default [
	{
		files: ["src/**/*.ts"],
		ignores: ["src/**/*.test.ts", "src/date.ts"],
		languageOptions: {
			parser: parser,
			parserOptions: {
		...
		rules: {
			...
			"no-restricted-syntax": [
				"error",
				{
					...
					message: "Use date-fns functions via @seventysixcommerce/shared/date instead of new Date()."
				},
				{
					selector: "MemberExpression[object.name='Date'][property.name='now']",
					message: "Use date-fns functions via @seventysixcommerce/shared/date instead of Date.now()."
				},
				{
					selector: "MemberExpression[object.name='Date'][property.name='parse']",
					message: "Use date-fns parse() via @seventysixcommerce/shared/date instead of Date.parse()."
				}
			]
		}
	},
