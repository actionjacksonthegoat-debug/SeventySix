	...
	IOptions<PollyOptions> options) : IPollyIntegrationClient
{
		...
		where T : class
	{
		...
		try
		{
			...
			HttpResponseMessage response = await ResiliencePipeline.ExecuteAsync(
				async cancellation =>
				{
					HttpResponseMessage httpResponse = await httpClient.GetAsync(url, cancellation);

					// Increment rate limit counter on successful request
					await rateLimitingService.TryIncrementRequestCountAsync(apiName, baseUrl, cancellation);
		...
		{
			Logger.LogError(exception, "Circuit breaker is open for {ApiName}. Service may be unavailable.", apiName);
		...
		{
			Logger.LogError(exception, "Request timeout for {ApiName} after {TimeoutSeconds}s", apiName, options.Value.TimeoutSeconds);
			throw;
		}
		catch (HttpRequestException exception)
		{
			Logger.LogError(exception, "HTTP request failed for {ApiName}: {Message}", apiName, exception.Message);
			throw;
		}
	}
		...
		where TResponse : class
	{
		...
		try
		{
			...
			HttpResponseMessage response = await ResiliencePipeline.ExecuteAsync(
				async cancellation =>
				{
					...
					HttpResponseMessage httpResponse = await httpClient.PostAsync(url, content, cancellation);
		...
		{
			Logger.LogError(exception, "POST request failed for {ApiName}", apiName);
			throw;
		}
	}
	...
	public async Task<bool> CanMakeRequestAsync(string apiName, CancellationToken cancellationToken = default) =>
		await rateLimitingService.CanMakeRequestAsync(apiName, cancellationToken);
