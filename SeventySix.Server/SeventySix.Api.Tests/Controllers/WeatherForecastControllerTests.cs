...
/// </summary>
public class WeatherForecastControllerTests : PostgreSqlTestBase, IClassFixture<PostgreSqlFixture>
{
	...
	public WeatherForecastControllerTests(PostgreSqlFixture fixture)
		: base(fixture)
	{
	...
	private void SetupDefaultMockBehavior()
	{
		...
		MockApiClient
			.Setup(c => c.CanMakeRequestAsync(It.IsAny<CancellationToken>()))
			.ReturnsAsync(true);

		MockApiClient
			.Setup(c => c.GetRemainingQuotaAsync(It.IsAny<CancellationToken>()))
			.ReturnsAsync(1000);

		OneCallResponse defaultResponse = new()
		{
		...
		MockApiClient
			.Setup(c => c.GetHistoricalDataAsync(It.IsAny<HistoricalWeatherRequest>(), It.IsAny<CancellationToken>()))
			.ReturnsAsync(defaultResponse);
	}
