	...
	where TEntity : class
{
	...
	protected BaseQueryValidator()
	{
		...
		When(
			request => !string.IsNullOrWhiteSpace(request.SearchTerm),
			() =>
			{
		...
		When(
			request => request.StartDate.HasValue && request.EndDate.HasValue,
			() =>
			{
		...
		When(
			request => !string.IsNullOrWhiteSpace(request.SortBy),
			() =>
			{
