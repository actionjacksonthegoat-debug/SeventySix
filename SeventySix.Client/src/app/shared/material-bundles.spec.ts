...
describe("Material Bundles", () =>
{
	...
	it("FORM_MATERIAL_MODULES should include form essentials", () =>
	{
		const moduleNames: string[] =
			FORM_MATERIAL_MODULES.map((module: Type<unknown>) =>
				module.name || "");

		// Should include core form modules
		expect(moduleNames.some((name: string) =>
			name.includes("FormField")))
			.toBe(true);
		expect(moduleNames.some((name: string) => name.includes("Input")))
			.toBe(true);
		expect(moduleNames.some((name: string) =>
	...
	{
		const moduleNames: string[] =
			TABLE_MATERIAL_MODULES.map((module: Type<unknown>) =>
				module.name || "");

		// Should include core table modules
		expect(moduleNames.some((name: string) => name.includes("Table")))
			.toBe(true);
		expect(moduleNames.some((name: string) =>
			name.includes("Paginator")))
			.toBe(true);
		expect(moduleNames.some((name: string) => name.includes("Sort")))
			.toBe(true);
	});
});
