...
describe("Table Models", () =>
{
	...
	describe("TableColumn<T>", () =>
	{
		...
		it("should define a valid badge column with color function", () =>
		{
			const column: TableColumn<TestEntity> = {
				...
				visible: true,
				type: "badge",
				badgeColor: (value: unknown, row?: TestEntity) =>
					value === "active" ? "primary" : "warn"
			};

			expect(column.badgeColor?.("active")).toBe("primary");
			expect(column.badgeColor?.("inactive")).toBe("warn");
		});
		...
		it("should support formatter with row context", () =>
		{
			const column: TableColumn<TestEntity> = {
				...
				sortable: true,
				visible: true,
				formatter: (value: unknown, row?: TestEntity) =>
					`${value} (${row?.status})`
			};
			...
			expect(column.formatter?.("Test", testRow)).toBe("Test (active)");
		});
	});
	...
	describe("QuickFilter<T>", () =>
	{
		it("should define a valid filter with predicate", () =>
		{
			...
			expect(filter.filterFn).toBeDefined();
			expect(filter.filterFn?.(activeItem)).toBe(true);
			expect(filter.filterFn?.(inactiveItem)).toBe(false);
		});
	...
	describe("RowAction<T>", () =>
	{
		...
		it("should support conditional visibility with showIf", () =>
		{
			const action: RowAction<TestEntity> = {
				...
				label: "Activate",
				icon: "check",
				showIf: (row: TestEntity) => row.status === "inactive"
			};
			...
			expect(action.showIf?.(activeRow)).toBe(false);
			expect(action.showIf?.(inactiveRow)).toBe(true);
		});
	...
	describe("Type Safety", () =>
	{
		it("should enforce generic type constraints", () =>
		{
			...
			expect(column.formatter?.(1, testRow)).toBe("Test");
		});
		...
		it("should support unknown generic for flexible usage", () =>
		{
			...
			expect(genericColumn.key).toBe("data");
		});
	});
});
