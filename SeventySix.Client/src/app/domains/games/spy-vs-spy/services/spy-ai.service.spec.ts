...
describe("SpyAiService",
	() =>
	{
		...
		describe("room coverage tracking",
			() =>
			{
				it("should not revisit a recently visited room immediately",
					() =>
					{
						service.initialize(scene, aiNode);

						/* Place AI at Watchtower center (28, -20) — its spawn room. */
						...
						/* Force 100% wander by mocking Math.random to return value below wander threshold. */
						const randomSpy: ReturnType<typeof vi.spyOn> =
							vi
								.spyOn(Math, "random")
								.mockReturnValue(0.01);

						/* Run multiple decision cycles. Track rooms visited. */
						const visitedRooms: RoomId[] = [];
						...
						for (let cycle: number = 0; cycle < 6; cycle++)
						{
							...
							{
								visitedRooms.push(state.currentRoomId);
							}
						}

						randomSpy.mockRestore();

						/* With recent-room exclusion, AI should not repeatedly pick the same room. */
						let consecutiveSameRoom: number = 0;

						for (let idx: number = 1; idx < visitedRooms.length; idx++)
						{
							if (visitedRooms[idx] === visitedRooms[idx - 1])
							{
								consecutiveSameRoom++;
							}
						}

						/* Allow some since AI might be in transit, but strict consecutive
						   same-room picks across 6 cycles should be rare with exclusion. */
						expect(consecutiveSameRoom, "AI should not always pick same room")
							.toBeLessThan(5);
					});
				...
				it("should track fully searched rooms and prefer unsearched rooms",
					() =>
					{
						...
						/* Mark all furniture in Watchtower as searched by AI. */
						const watchtowerFurniture: string[] =
							ROOM_FURNITURE
								.filter(
									(furn) =>
										furn.roomId === RoomId.Watchtower)
								.map(
									(furn) => furn.id);

						for (const furnId of watchtowerFurniture)
						{
							service.markFurnitureSearched(furnId);
						}

						/* After marking all Watchtower furniture searched,
						   AI should prefer other rooms when deciding next goal. */
						const playerState: ReturnType<typeof createPlayerState> =
							createPlayerState();
						playerState.positionX = -28;
						playerState.positionZ = -20;

						/* Place AI at Watchtower. */
						aiNode.position.x = 28;
						aiNode.position.z = -20;

						/* Force wander decisions. */
						const randomSpy: ReturnType<typeof vi.spyOn> =
							vi
								.spyOn(Math, "random")
								.mockReturnValue(0.01);
						...
						/* Just verify it doesn't crash and AI is still functional. */
						expect(service.getState().identity)
							.toBe(SpyIdentity.White);
					});
				...
				it("should reset fully searched rooms when collecting an item",
					() =>
					{
						...
						/* Mark all Watchtower furniture as searched. */
						const watchtowerFurniture: string[] =
							ROOM_FURNITURE
								.filter(
									(furn) =>
						...
						/* Collect an item — should reset fully searched tracking. */
						service.collectItem(ItemType.SecretDocuments);

						/* AI should still function normally. */
						const playerState: ReturnType<typeof createPlayerState> =
							createPlayerState();
			...
			() =>
			{
				it("should increase aggression when player is in same room",
					() =>
					{
						...
						/* Place both AI and player in Watchtower (28, -20). */
						aiNode.position.x = 28;
						aiNode.position.z = -20;
						...
						/* Force random to trigger interception (below AI_INTERCEPT_PROBABILITY=0.30). */
						const randomSpy: ReturnType<typeof vi.spyOn> =
							vi
								.spyOn(Math, "random")
						...
						/* AI should remain functional and in Watchtower area. */
						const state: ReturnType<SpyAiService["getState"]> =
							service.getState();
				...
				it("should consider intercepting player in adjacent room",
					() =>
					{
						...
						/* AI in Watchtower (28, -20), player in JungleHQ (0, -20) — connected rooms. */
						aiNode.position.x = 28;
						aiNode.position.z = -20;
						...
						/* Force interception probability. */
						const randomSpy: ReturnType<typeof vi.spyOn> =
							vi
								.spyOn(Math, "random")
								.mockReturnValue(0.10);
		...
		describe("reduced AI timers",
			() =>
			{
				it("AI_DECISION_INTERVAL_SECONDS should be 2.0",
					() =>
					{
				...
				it("should re-evaluate more frequently with lower decision interval",
					() =>
					{
						service.initialize(scene, aiNode);
						...
						/* At 2.0s interval, a 2.1s tick should trigger re-evaluation. */
						const randomSpy: ReturnType<typeof vi.spyOn> =
							vi
								.spyOn(Math, "random")
								.mockReturnValue(0.99);
						...
						/* AI should have moved from spawn. */
						const state: ReturnType<SpyAiService["getState"]> =
							service.getState();
						const distFromSpawn: number =
							Math.sqrt(
								(state.positionX - WHITE_SPY_SPAWN_X) ** 2
									+ (state.positionZ - WHITE_SPY_SPAWN_Z) ** 2);

						expect(distFromSpawn, "AI should have moved after 2.1s tick")
							.toBeGreaterThan(0);
