...
describe("buildHttpParams", () =>
{
	...
	{
		const result: HttpParams = buildHttpParams({});
	...
	{
		const result: HttpParams = buildHttpParams({ name: "test" });
	...
	{
		const result: HttpParams = buildHttpParams({ page: 1, size: 10 });
	...
	it("should handle Date values as ISO strings", () =>
	{
		...
		const result: HttpParams = buildHttpParams({ createdAt: date });

		expect(result.get("createdAt")).toBe("2024-01-15T10:30:00.000Z");
	});
	...
	{
		const date: Date = new Date("2024-06-01T00:00:00.000Z");
		const result: HttpParams = buildHttpParams({
			page: 1,
			search: "test",
			active: true,
		...
		expect(result.get("active")).toBe("true");
		expect(result.get("startDate")).toBe("2024-06-01T00:00:00.000Z");
		expect(result.has("nullValue")).toBe(false);
		expect(result.has("undefinedValue")).toBe(false);
	});
});
