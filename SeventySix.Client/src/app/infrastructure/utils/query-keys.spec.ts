...
describe("QueryKeys", () =>
{
	describe("logs", () =>
	{
		...
		{
			const key: readonly string[] = QueryKeys.logs.all;
			expect(key).toEqual(["logs"]);
		});
	...
	describe("users", () =>
	{
		it("should have correct all key", () =>
		{
			const key: readonly string[] = QueryKeys.users.all;
		...
		{
			const key: readonly unknown[] = QueryKeys.users.single(123);
			expect(key).toEqual(["users", "user", 123]);
		});

		it("should generate correct single key with string id", () =>
		{
			const key: readonly unknown[] = QueryKeys.users.single("abc");
			expect(key).toEqual(["users", "user", "abc"]);
		});
		...
		it("should generate correct byUsername key", () =>
		{
			...
			expect(key).toEqual(["users", "username", "testuser"]);
		});
	});
	...
	describe("type safety", () =>
	{
		it("should return readonly arrays", () =>
		{
			...
			const logsAll: readonly string[] = QueryKeys.logs.all;
			const usersAll: readonly string[] = QueryKeys.users.all;

			expect(logsAll).toBeDefined();
