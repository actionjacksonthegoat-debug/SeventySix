import type { LogEntry } from "$lib/server/log-forwarder";
import {
	_resetForTesting,
	...
	() =>
	{
		...
		describe("forwardLogs",
			() =>
			{
				...
				it("sets SourceContext to 'seventysixcommerce-sveltekit'",
					async () =>
					{
						const mockFetch =
							vi
			...
			() =>
			{
				...
				it("queues entries and flushes when batch size reached",
					async () =>
					{
						...
						// Allow microtask (flush is async via void)
						await vi.waitFor(
							() =>
							{
								expect(mockFetch)
