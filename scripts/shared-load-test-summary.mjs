/**
 * Shared Load Test Summary Report Generator
 *
 * Reads k6 result JSON files from a results directory and generates
 * a combined HTML summary with a table of all scenarios and their pass/fail status.
 *
 ...
 *
 * @module shared-load-test-summary
 */
 ...
 * @param {object} source - The source object to traverse.
 * @param {string[]} keys - The keys to traverse.
 ...
 * @param {object} data - The k6 result data.
 * @returns {{ totalPasses: number, totalFails: number }} The check summary.
 */
function extractChecks(data)
{
	...
	/**
	 * Recursively walk k6 groups to accumulate check results.
	 * @param {object} group - The k6 group to walk.
	 */
...
/**
 * Generates an HTML summary report from k6 load test result JSON files.
 * @param {object} config - Report generation configuration.
 * @param {string} config.resultsDir - Absolute path to the k6 results directory.
 * @param {string} config.reportsDir - Absolute path to write the generated report.
 * @param {string} config.title - Title for the HTML report (e.g., "SeventySix Load Test Summary").
 * @returns {void}
 */
export function generateSummary(config)
{
	const { resultsDir, reportsDir, title } = config;
	const outputFile =
		path.join(reportsDir, "summary.html");
			...
			scenarios.push(
				{
					...
					rps: reqsValues.rate != null ? reqsValues.rate.toFixed(1) : "N/A",
...
<body>
	...
	console.log(`Summary report generated: ${outputFile}`);
	console.log(
