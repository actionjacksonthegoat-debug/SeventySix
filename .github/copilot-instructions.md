...
## ⚠️ CRITICAL: IDE and Analyzer Warnings (NEVER IGNORE)

**RULE:** All IDE, EditorConfig, and analyzer warnings MUST be **fixed properly** - NEVER suppressed or ignored.

| ❌ NEVER                                | ✅ ALWAYS                                     |
| --------------------------------------- | --------------------------------------------- |
| `#pragma warning disable`               | Fix the root cause                            |
| `// <auto-generated />` to bypass rules | Format code to comply with rules              |
| `[SuppressMessage(...)]`                | Refactor to eliminate warning                 |
| `.editorconfig` with `severity = none`  | Only for truly auto-generated files (EF Core) |
| Ignoring yellow squiggles               | Address every warning before committing       |

**Exceptions** (approved suppression patterns):

- **EF Core Migrations**: Auto-generated by `dotnet ef migrations add` - formatting rules disabled in `.editorconfig` for `**/Migrations/*.cs`
- **Generated OpenAPI clients**: Auto-generated code from external tools

**Process for Warnings:**

1. **Read** the warning message carefully
2. **Understand** what rule is being violated
3. **Fix** by reformatting/refactoring code to comply
4. **Verify** the warning is gone before committing
...
**Migration Commands** (from `SeventySix.Server/SeventySix.Domains/`):
