...
export default [
	{
		files: ["src/**/*.{ts,tsx}"],
		ignores: ["src/**/*.test.{ts,tsx}", "src/lib/date.ts"],
		languageOptions: {
			parser: parser,
			parserOptions: {
		...
		rules: {
			...
			"no-restricted-syntax": [
				"error",
				{
					...
					message: "Use date-fns functions via ~/lib/date instead of new Date()."
				},
				{
					selector: "MemberExpression[object.name='Date'][property.name='now']",
					message: "Use date-fns functions via ~/lib/date instead of Date.now()."
				},
				{
					selector: "MemberExpression[object.name='Date'][property.name='parse']",
					message: "Use date-fns parse() via ~/lib/date instead of Date.parse()."
				}
			]
		}
	},
	...
	{
		files: ["src/routes/**/*.{ts,tsx}", "src/server/functions/**/*.ts", "src/server/middleware/**/*.ts", "src/server.ts"],
