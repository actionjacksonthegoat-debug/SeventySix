#!/usr/bin/env bash
# run-load-tests.sh
# Linux/CI orchestrator for SeventySixCommerce TanStack load tests
#
# Usage:
...
echo ""
echo "========================================"
echo "  TanStack Load Tests ($PROFILE)"
echo "========================================"
echo ""
...
echo ""
echo "Starting TanStack load test environment..."
cd "$REPO_ROOT"
docker compose -f docker-compose.loadtest-tanstack.yml up -d --build

# 3. Wait for app health check
echo ""
echo "Waiting for TanStack app to become healthy..."
MAX_ATTEMPTS=60
ATTEMPT=0
APP_HEALTHY=0
...
while [ "$ATTEMPT" -lt "$MAX_ATTEMPTS" ]; do
	ATTEMPT=$((ATTEMPT + 1))
	HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:3022/api/healthz" 2>/dev/null || echo "000")
...
if [ "$KEEP_RUNNING" != "1" ]; then
	...
	echo "Stop manually: docker compose -f docker-compose.loadtest-tanstack.yml down"
fi
