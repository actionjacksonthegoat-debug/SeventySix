...
**JWT signing key** (auto-generated by `secrets:init` — change if rotating keys):

The application signs access tokens with HS256. The secret is auto-generated during bootstrap.
For graceful key rotation (e.g., after a secret leak), set the *previous* key first so existing
tokens remain valid while new tokens are issued with the new key:

```bash
# Step 1 — move current key to PreviousSecretKey before changing it
npm run secrets:set -- -Key "Jwt:PreviousSecretKey" -Value "your-current-secret"
npm run secrets:set -- -Key "Jwt:PreviousKeyId" -Value "your-current-key-id"

# Step 2 — set the new key (all new tokens will use this)
npm run secrets:set -- -Key "Jwt:SecretKey" -Value "your-new-secret-min-32-chars"
npm run secrets:set -- -Key "Jwt:KeyId" -Value "your-new-key-id"
```

After all existing tokens expire (default: 15-minute access token TTL), remove the previous-key entries:

```bash
npm run secrets:set -- -Key "Jwt:PreviousSecretKey" -Value ""
npm run secrets:set -- -Key "Jwt:PreviousKeyId" -Value ""
```
